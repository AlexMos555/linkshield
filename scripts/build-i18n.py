...
# Android-native strings for the VPN service (notifications live in the
# service, which runs when no JS is alive — so they cannot come from i18next).
ANDROID_RES_DIR = ROOT / "mobile" / "modules" / "cleanway-vpn" / "android" / "src" / "main" / "res"
...
def android_strings_xml(source: dict[str, Any]) -> str | None:
    ...
    lines = ['<?xml version="1.0" encoding="utf-8"?>',
             "<!-- GENERATED by scripts/build-i18n.py from packages/i18n-strings/src — do not edit. -->",
             "<resources>"]
...
def main() -> int:
    ...
    android_written = 0
    for loc in SUPPORTED_LOCALES:
        xml = android_strings_xml(sources[loc])
        if xml is None:
            continue
        values_dir = ANDROID_LOCALE_DIR.get(loc, f"values-{loc}")
        write_text_atomic(ANDROID_RES_DIR / values_dir / "strings.xml", xml)
        android_written += 1
    total_written += android_written
    ...
    print(f"  {android_written} android res/values*/strings.xml")
