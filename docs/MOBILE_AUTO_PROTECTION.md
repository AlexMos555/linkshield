...
| CONFLICT | Amber | Android: "Your phone has Private DNS turned on (dns.google). Android can't run it and Cleanway together — with both on, the internet stops working. Set Private DNS to Automatic…" + "Open network settings". | **Android (shipped 2026-08-18, emulator-verified):** strict Private DNS detected (`PrivateDnsGuard`: Settings.Global `private_dns_mode==hostname` OR `LinkProperties.privateDnsServerName`). Service refuses to establish, and steps aside within ~1s if the setting flips while running (ContentObserver) — because strict DoT + our tunnel = `PrivateDnsBroken` = no DNS for any app. Competing-VPN detection is still NOT implemented; nothing may claim it. |
